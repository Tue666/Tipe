...
class OrdersAPI {
  ...
  async findByStatus(req, res, next) {
    try {
      ...
      if (typeof status !== 'undefined') filter.$and.push({ 'tracking_info.status': status });
    ...
    try {
      ...
      switch (new_status) {
        ...
        case 'canceled':
          ...
          if (current.tracking_info.status !== 'processing') {
      ...
      const order = await Order.findOneAndUpdate(
        ...
        },
        {
          ...
          'tracking_info.status_text': status_text,
          'tracking_info.time': Date.now(),
          note,
        },
        {
          new: true,
