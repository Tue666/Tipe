...
class PaymentAPI {
  ...
  async momoCreate(req, res, next) {
    try {
      ...
      let requestBody = {
        ...
        orderInfo: `Pay the bill for ${phone_number}. Amount ${amount} VND on ${dayjs(
          Date.now()
  ...
  async momoIPNCallback(req, res, next) {
    try {
      ...
      if (resultCode === 0) {
        editBody['tracking_info.status'] = 'processing';
        editBody['tracking_info.status_text'] = 'Pending processing';
        editBody['tracking_info.time'] = Date.now();
      }
      const _id = orderId.split('-')[0];
      await Order.findOneAndUpdate(
        {
          _id,
          'tracking_info.status': 'awaiting_payment',
        },
        editBody
      );
    ...
    try {
      ...
      const editBody = {
        'payment_method.message':
          vnp_ResponseCode === '00' ? 'Giao dịch thành công' : 'Giao dịch thất bại',
